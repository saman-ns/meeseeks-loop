...
export function detectProject(workDir = process.cwd()): DetectedProject {
	...
	// Check for pubspec.yaml (Dart/Flutter) — must come before package.json
	// since Flutter projects may also contain a package.json
	const pubspecPath = join(workDir, "pubspec.yaml");
	if (existsSync(pubspecPath)) {
		detectDartProject(pubspecPath, result);
		return result;
	}
...
function detectDartProject(pubspecPath: string, result: DetectedProject): void {
	result.language = "Dart";
	result.testCmd = "dart test";
	result.lintCmd = "dart analyze";
	result.buildCmd = "";

	try {
		const content = readFileSync(pubspecPath, "utf-8");

		// Extract name from pubspec.yaml (simple YAML parsing)
		const nameMatch = content.match(/^name:\s*(.+)$/m);
		if (nameMatch) {
			result.name = nameMatch[1].trim();
		}

		// Detect Flutter: check for flutter sdk dependency
		const isFlutter =
			content.includes("sdk: flutter") ||
			(content.includes("flutter:") && content.includes("uses-material-design"));

		if (isFlutter) {
			result.framework = "Flutter";
			result.testCmd = "flutter test";
			result.lintCmd = "flutter analyze";
			result.buildCmd = "flutter build";
		}
	} catch (error) {
		logDebug("Failed to parse pubspec.yaml:", error);
	}
}
