...
export async function showConfig(workDir = process.cwd()): Promise<void> {
	if (!isInitialized(workDir)) {
		logWarn("No config found. Run 'meeseeks --init' first.");
		return;
	}
	...
	console.log("");
	console.log(`${pc.bold("Meeseeks Configuration")} (${configPath})`);
	console.log("");
	...
	if (config.rules.length > 0) {
		...
		console.log(`  ${pc.dim('(none - add with: meeseeks --add-rule "...")')}`);
	}
	console.log("");
...
export async function addRule(rule: string, workDir = process.cwd()): Promise<void> {
	if (!isInitialized(workDir)) {
		logError("No config found. Run 'meeseeks --init' first.");
		return;
	}
